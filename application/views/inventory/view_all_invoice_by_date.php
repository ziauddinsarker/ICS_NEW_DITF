...
<div class="col-md-8 col-sm-16 col-xs-18">
    <div>
        ...
        <form class="form-inline" role="form" action="<?=  base_url()?>inventory/all_invoice_by_date" method="post">
            From: <input type="text" class="form-control" value="<?php echo $show_from_date ?>" name="fromdate" id="fromdate" placeholder="From Date" required="required">
            To: <input type="text" class="form-control" value="<?php echo $show_to_date ?>" name="todate" id="todate" placeholder="To Date" onchange="this.form.submit()" required="required">
            <noscript><input type="submit" value="Submit"></noscript>
        </form>

        <form class="form-inline" role="form" action="<?=  base_url()?>inventory/gettotalreportbydate" method="post">
            <input  type="hidden" class="form-control" name="fromdatereport" id="fromdatereport" value="<?php echo $show_from_date ?>" >
            <input  type="hidden" class="form-control" name="todatereport" id="todatereport" value="<?php echo $show_to_date ?>" >
    ...
    <table class="table table-bordered table-hover">
        ...
        <h4>Showing data for : <?php if(!($show_from_date ==  '' || $show_from_date ==  '')){
                echo $show_from_date .' to ' . $show_to_date;
            }else {
                $date = date("Y-m-d");
                echo $date;
            }
