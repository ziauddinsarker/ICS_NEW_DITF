...
*/
    ...
    /*******************Product Fabric*********************/
		...
		function get_all_invoice_by_date($offset, $limit, $fromdate, $todate )
        {
	...
	function gettotalCSVByDate($fromdate = '',$todate = '')
        {
            if(!isset($fromdate,$todate)) {
                $fromdate = date("Y-m-d");
                $todate = date("Y-m-d");
            ...
            $sql = "SELECT
                    ...
                    WHERE
						tbl_orderdetail.date >= '" . $fromdate ."'
						AND
						tbl_orderdetail.date <= '" . $todate ."'
                    GROUP BY
                    customer_id";
        ...
        function product_left_on_inventory($product_code = ''){
            $this->db->select('tbl_product.id,tbl_product.product_code,tbl_inventory.product_left,tbl_inventory.product_sold');
            $this->db->from('tbl_inventory');
            $this->db->join('tbl_product','tbl_inventory.product_id = tbl_product.id');
